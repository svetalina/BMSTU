...
def pfe_inputs(root):
    t = tk.Label(root, text="Эксперимент", background=color1)
    t.grid(column=1)
    ...
    i_list_1.pack(side=LEFT, padx=0)
    i_list_2.pack(side=LEFT,  padx=0)
    i_list_3.pack(side=LEFT,  padx=0)
    i_list_4.pack(side=LEFT,  padx=0)
    ...
    btn = Button(root, text="Запуск", bg=color2, fg="white")
...
def draw_new_point(root):
    ...
    btn = Button(root, text="Добавить", state=DISABLED, bg=color2, fg="white")
...
if __name__ == '__main__':
    ...
    f_pfe = Frame(root, background=color1)
    f_one = Frame(root, background=color1)
    pfe_inputs(f_pfe)
