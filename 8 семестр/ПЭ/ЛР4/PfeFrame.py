...
class PfeFrame(tk.Frame): 
    def __init__(self, master): 
        ...
        self.not_lin_nat_formula = tk.StringVar()
        self.a_string = tk.StringVar()
        self.alpha_string = tk.StringVar()
        a_label = tk.Label(  self.formula_frame, text="a: ")
        ...
        alpha_label = tk.Label(  self.formula_frame, text="Звездное плечо (alpha): ")
        ...
        not_lin_formula_label = tk.Label(self.formula_frame, textvariable=self.not_lin_nat_formula)
    ...
    def run(self, lambda_min, lambda_max, mu_min, mu_max, count, lambda2_min, lambda2_max, mu2_min, mu2_max):
        ...
        N0 = 2**(lin_count)
        N = N0 + 2 * lin_count + 1
        self.a = a = math.sqrt( N0 / N) # = 16/25 = 0.8
        self.alpha = alpha = math.sqrt(1/2 * (math.sqrt( N * N0 ) - N0 )) # = 1.4142
        ...
        if lambda_min == lambda2_min and lambda_max == lambda2_max:
            b[3] = b[1] # 1 = 3
            b[10] = b[7] # 34 = 14
            b[12] = b[14] # 124 = 234
        
        if mu_min == mu2_min and mu_max == mu2_max:
            b[4] = b[2] # 2 = 4
            b[5] = b[7] # 12 = 14
            b[13] = b[11] # 123 = 134
        
        if lambda_min == lambda2_min and lambda_max == lambda2_max and mu_min == mu2_min and mu_max == mu2_max:
            b[8] = b[7] # 23 = 14


        # for i in range(len(b)):
        #     b[i] = math.trunc(b[i] * 10**5) / 10**5
        ...
        for i in range (1, len(b)): 
            ...
            if (i in inx_negative):
                not_lin_str += " - " + str('{:.5f}'.format(math.fabs(b[i]))) + " * x" + x_indexes[i]
                continue
            ...
            if (b[i] > 0):
                not_lin_str += " + " + str('{:.5f}'.format(b[i])) + " * x" + x_indexes[i]
            else: 
                not_lin_str += " - " + str('{:.5f}'.format(math.fabs(b[i]))) + " * x" + x_indexes[i]
        # print(not_lin_str)
        ...
        max_complex_factors = [self.lambda_max, self.mu_max, self.lambda2_max, self.mu2_max,
                               self.lambda_max * self.mu_max, self.lambda_max * self.lambda2_max, self.lambda_max * self.mu2_max,
                               self.mu_max * self.lambda2_max, self.mu_max * self.mu2_max,
                               self.lambda2_max * self.mu2_max,
                               self.lambda_max * self.mu_max * self.lambda2_max,
                               self.lambda_max * self.mu_max * self.mu2_max,
                               self.lambda_max * self.lambda2_max * self.mu2_max,
                               self.mu_max * self.lambda2_max * self.mu2_max,
                               self.lambda_max * self.mu_max * self.lambda2_max * self.mu2_max,
                               self.lambda_max**2, self.mu_max**2, self.lambda2_max**2, self.mu2_max**2]

        min_complex_factors = [self.lambda_min, self.mu_min, self.lambda2_min, self.mu2_min,
                               self.lambda_min * self.mu_min, self.lambda_min * self.lambda2_min, self.lambda_min * self.mu2_min,
                               self.mu_min * self.lambda2_min, self.mu_min * self.mu2_min,
                               self.lambda2_min * self.mu2_min,
                               self.lambda_min * self.mu_min * self.lambda2_min,
                               self.lambda_min * self.mu_min * self.mu2_min,
                               self.lambda_min * self.lambda2_min * self.mu2_min,
                               self.mu_min * self.lambda2_min * self.mu2_min,
                               self.lambda_min * self.mu_min * self.lambda2_min * self.mu2_min,
                               self.lambda_min**2, self.mu_min**2, self.lambda2_min**2, self.mu2_min**2]

        b_nat = self.calculate_natural_coefficients_regression_equation(max_complex_factors, min_complex_factors, b)
        print("\n\n B", b)
        print("\n\n BN", b_nat)
        
        not_lin_nat_str = "y = " + str('{:.5g}'.format(b_nat[0]))
        for i in range (1, len(b_nat)): 
            if i == round(len(b_nat)/2):
                not_lin_nat_str += '\n'
            
            if (i in inx_negative):
                not_lin_nat_str += " - " + str('{:.5g}'.format(math.fabs(b_nat[i]))) + " * x" + x_indexes[i]
                continue
            if (i in inx_positive):
                not_lin_nat_str += " + " + str('{:.5g}'.format(math.fabs(b_nat[i]))) + " * x" + x_indexes[i]
                continue
            ...
            if (b[i] > 0):
                not_lin_nat_str += " + " + str('{:.5g}'.format(b_nat[i])) + " * x" + x_indexes[i]
            else: 
                not_lin_nat_str += " - " + str('{:.5g}'.format(math.fabs(b_nat[i]))) + " * x" + x_indexes[i]
        # print(not_lin_str)

        self.not_lin_nat_formula.set(not_lin_nat_str)
        
    def calculate_natural_coefficients_regression_equation(self, max_complex_factors, min_complex_factors, coefficients_normalized):
        coefficients_natural = [0] * len(coefficients_normalized)

        coefficients_natural[0] = coefficients_normalized[0]
        for i in range(1, len(coefficients_natural)):
            coefficients_natural[0] -= coefficients_normalized[i] * (
                        max_complex_factors[i - 1] + min_complex_factors[i - 1]) / (
                                                   max_complex_factors[i - 1] - min_complex_factors[i - 1])

        for i in range(1, len(coefficients_normalized)):
            coefficients_natural[i] = 2 * coefficients_normalized[i] / (
                        max_complex_factors[i - 1] - min_complex_factors[i - 1])

        return coefficients_natural
