...
def work_view(Event):
    view(
        ...
        step=float(varList["step"].get()), 
        N=float(varList["N_exp"].get()),
        Exps=float(varList["N_count"].get()),
        lambda_com=float(varList["lambda"].get()),
        lambda_obr=float(varList["mu"].get())
    )
...
def one_model_list(root):
    items = [
        i.Item(text="Интенсивность поступления заявок:", var=varList["lambda"], value=35),
    ...
    btn.configure(font=FONT, bg=ADDED_COLOR, fg=MAIN_COLOR)
    btn.grid(column=1, padx=10, pady=10)


def expirement_list(root):
    items = [
        i.Item(text="Интенсивность (мин.):", var=varList["start"], value=0.01),
        i.Item(text="Интенсивность (макс.):", var=varList["end"], value=1.0),
        i.Item(text="Интенсивность (шаг):", var=varList["step"], value=0.05),
        i.Item(text="Число экспериментов:", var=varList["N_count"], value=100),
    ...
    btn2.configure(font=FONT, bg=ADDED_COLOR, fg=MAIN_COLOR)
