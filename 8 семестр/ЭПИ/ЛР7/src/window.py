...
from fp import calculateFP, adjustFP, getLocByFP
...
class Window(QMainWindow):
    ...
    def funcDotsMethod(self):
        ...
        if (funcDotsTableMatrix is None): 
            return

        # 2. Calculate
        fp = calculateFP(funcDotsTableMatrix)
        afp = adjustFP(fp[-1], productAttributes)
        loc = getLocByFP(afp, languagePercents)
        self.loc = loc
