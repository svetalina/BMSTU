...
def main():
    ...
    while True:
        ...
        if test == 2:
            ...
            F0 = float(input("Введите F0 (по умолчанию F0 = 50): "))
            N = int(input("Введите число точек (по умолчанию N = 100): "))
            coeff = float(input("Введите во сколько раз изменить a0 (по умолчанию a0 = 0.0194): "))
            a0 = a0 * coeff
        elif test == 0:
            return
        elif test == 1:
            ...
            print("Некорректный номер теста!")
            return
        ...
        ax.plot(x, T, '-r')
        ax.set_xlabel('Расстояние от левого торца стержня, см', fontsize=10)
        ax.set_ylabel('Температура, К', fontsize=10)
        plt.show()
